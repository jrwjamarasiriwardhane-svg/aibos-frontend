...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // #region agent log
      fetch('http://127.0.0.1:7468/ingest/40b9b3d1-81e1-44a0-9a4c-9da7cea37d4d',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'c1632a'},body:JSON.stringify({sessionId:'c1632a',runId:'post-fix',hypothesisId:'E',location:'professional/LoginPage.tsx:handleSubmit',message:'professional login response',data:{ok:response.ok,status:response.status,requiresEmailVerification:Boolean(data.requiresEmailVerification),willRedirectToVerify:Boolean(!response.ok && data.requiresEmailVerification),role:data.user?.role||null,message:data.message},timestamp:Date.now()})}).catch(()=>{});
      // #endregion
            ...
            {error && (
              ...
              >
                ...
                {error.toLowerCase().includes("verify") && (
                  <Link
                    to={`/verify-email?email=${encodeURIComponent(formData.email)}&role=professional`}
